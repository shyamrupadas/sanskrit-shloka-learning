...
16. Как владелец приложения, я хочу использовать Railway-generated public domain до переноса frontend на VPS, чтобы backend можно было проверить без преждевременной настройки custom domains.
...
21. Как владелец приложения, я хочу временно разрешить точный origin production frontend на Netlify, чтобы проверить браузерный login и защищённые страницы до переноса frontend на VPS.
...
27. Как владелец приложения, я хочу проверить production backend через вход существующего пользователя и открытие защищённой страницы в production frontend на Netlify, чтобы одним smoke-check подтвердить public routing, CORS, auth и доступ к общей Neon DB.
...
34. Как владелец приложения, я хочу явно передать VPS effort обязанность заменить временный Netlify CORS origin на новый точный production HTTPS origin, чтобы браузерный доступ сохранился после переноса frontend.
...
- До первого запуска задаются `NODE_ENV=production`, точный временный `FRONTEND_ORIGIN=https://sanskrit-shloka-learning.netlify.app`, pooled `DATABASE_URL`, direct `DATABASE_DIRECT_URL` той же Neon DB и `DATABASE_POOL_MAX=5`. `PORT` вручную не задаётся.
...
- Первый deployment принимается только после проверки Railway-стадий и runtime logs, а затем ручного smoke-check через production frontend на Netlify с production API base URL. Smoke-check включает вход существующего пользователя и открытие одной защищённой страницы.
...
- Высший end-to-end шов для первого production deployment — ручной smoke-check уже активного Railway domain через production frontend на Netlify: вход существующего пользователя и открытие защищённой страницы. Этот сценарий одновременно подтверждает routing, точный CORS origin, auth, API contract на реальном запросе и доступ к Neon.
...
- Перенос frontend с Netlify на Nginx/VPS и настройка будущего публичного API URL.
...
- `FRONTEND_ORIGIN=https://sanskrit-shloka-learning.netlify.app` — текущий временный точный cross-origin allowlist, а не same-origin режим и не wildcard. Перенос frontend на Nginx/VPS обязан заменить его на новый точный production HTTPS origin и повторить login/CORS smoke-check.
