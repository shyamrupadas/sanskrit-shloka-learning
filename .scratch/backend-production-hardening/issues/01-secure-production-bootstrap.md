# 01 — Сделать production bootstrap fail-fast и безопасным

**What to build:** Провести один полный срез через production bootstrap API: приложение должно собрать и проверить конфигурацию до открытия HTTP-порта, запуститься на Railway-compatible host/port и сразу применять согласованные HTTP guardrails.
