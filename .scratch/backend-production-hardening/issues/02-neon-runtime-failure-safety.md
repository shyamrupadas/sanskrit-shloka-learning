# 02 — Сделать runtime-работу с Neon предсказуемой при сбоях

**What to build:** Провести полный failure-path от `pg` pool до HTTP-ответа: runtime использует небольшой Neon-compatible pool, безопасное чтение получает не более одной повторной попытки после классифицированного обрыва, а запись и query timeout никогда автоматически не повторяются.
...
`.scratch/backend-production-hardening/spec.md` — пользовательские истории 1–8, 14–19 и 35–37.
