...
- [x] Workflow для задач с видимым UI требует pen.dev references в формате human-readable name + `nodeId`.
...
- [x] Правила явно фиксируют, что `design/pen-design.pen` является каноническим UI-контрактом, а `design/new-design.pen` deprecated.
- [x] Правила объясняют, что функциональное поведение остается за spec, доменными документами, ADR и API-контрактом.
...
- [x] Актуальные pen.dev references получают напрямую через Pencil tools без статического навигационного индекса.
