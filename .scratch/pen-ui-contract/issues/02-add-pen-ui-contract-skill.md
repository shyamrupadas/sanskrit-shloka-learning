...
- [x] Design edit начинается непосредственно с MCP `get_app_state`; read-only CLI и его preflight не запускаются как prerequisite этой ветки.
...
- [x] Перед началом frontend code phase skill один раз читает релевантные узлы утвержденного дизайна через headless CLI; после design edit это происходит только после подтверждения владельца о приёмке и сохранении, а повторяется лишь после следующего принятого изменения.
