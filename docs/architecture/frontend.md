...
## Отложенный архитектурный вопрос

Нужно пересмотреть выбор оболочки маршрута: сейчас `AuthenticatedLayout`
определяет видимость нижнего меню и отступы по `pathname`. При добавлении
экранов список исключений растёт. Предлагается задавать оболочку в композиции
маршрутов слоя `app` и передавать лейауту явную конфигурацию.
Способ реализации пока не выбран; текущее поведение сохраняется до отдельной
задачи. Варианты и рекомендации собраны в
