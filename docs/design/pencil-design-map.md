...
- Рекомендация агента: обновить Pencil перед frontend-реализацией, потому что
  выбор влияет на плотность дашборда, состав блока и поведение action.
...
- Рекомендация агента: обновить Pencil или изменить требование на переход в
  библиотеку; не реализовывать inline-раскрытие молча, потому что оно меняет
  композицию и визуальную нагрузку дашборда.
