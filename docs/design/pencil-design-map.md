...
Эта карта не заменяет дизайн и не добавляет новые требования. Ее назначение — быстро находить человекочитаемые имена и `nodeId` для spec, tickets, frontend-задач, финальных отчетов и reviewer-проверки.

## Как использовать карту

- При подготовке spec или ticket с видимым UI выбрать релевантные экраны, состояния и reusable-компоненты из таблиц ниже и перенести их в блок `## Pencil references`.
...
| Product | Shloka card | `Product / Shloka Card` (`Vzs9b`) |
...
| Product Layout | Main layout with bottom navigation | `Product / Layout / Main With Bottom Navigation` (`Uwe4t`) |
