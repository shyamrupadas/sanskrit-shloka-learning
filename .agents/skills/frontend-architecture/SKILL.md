...
description: Audit, evolve, and migrate the apps/web frontend architecture. Use when Codex needs to review module boundaries or interfaces, move legacy frontend code into the current app/features/shared structure, choose a module evolution stage, or perform architecture-preserving frontend refactors. Do not use for a local product change that preserves module responsibility and interface.
---
...
4. Мигрировать снизу вверх: `shared` -> session/API infrastructure -> `app` composition -> `features` -> удаление legacy paths.
...
- Legacy paths не размножаются. Временные compatibility imports допустимы только внутри незавершенного шага и должны иметь понятный план удаления.
...
- Тесты переносить к публичному интерфейсу модуля; не закреплять приватные hooks, внутренние компоненты или временные пути.
...
- остались ли legacy paths или временные compatibility imports.
